...
class Autocomplete {
  constructor(element, callback, options) {
    ...
    this.container = import_leaflet.default.DomUtil.create("div", "leaflet-routing-geocoder-result");
    this.resultTable = import_leaflet.default.DomUtil.create("table", "", this.container);
    import_leaflet.default.DomEvent.addListener(this.element, "input", this.keyPressed, this);
    import_leaflet.default.DomEvent.addListener(this.element, "keypress", this.keyPressed, this);
    import_leaflet.default.DomEvent.addListener(this.element, "keydown", this.keyDown, this);
    import_leaflet.default.DomEvent.addListener(this.element, "blur", () => {
      if (this.isOpen) {
        this.close();
      }
    }, this);
  ...
  close() {
    import_leaflet.default.DomUtil.removeClass(this.container, "leaflet-routing-geocoder-result-open");
    this.isOpen = false;
  }
  open() {
    ...
    import_leaflet.default.DomUtil.addClass(this.container, "leaflet-routing-geocoder-result-open");
    this.isOpen = true;
  }
  setResults(results) {
    var _a;
    ...
    for (const result of results) {
      const tr = import_leaflet.default.DomUtil.create("tr", "", this.resultTable);
      tr.setAttribute("data-result-index", results.indexOf(result).toString());
      const td = import_leaflet.default.DomUtil.create("td", "", tr);
      let text = result.name;
      if (this.options.formatGeocoderResult) {
        text = this.options.formatGeocoderResult(result);
      }
      td.append(text);
      import_leaflet.default.DomEvent.addListener(td, "mousedown", import_leaflet.default.DomEvent.preventDefault, this);
      import_leaflet.default.DomEvent.addListener(td, "click", () => this.createClickListener(result), this);
    }
    if (!results.length) {
      const tr = import_leaflet.default.DomUtil.create("tr", "", this.resultTable);
      const td = import_leaflet.default.DomUtil.create("td", "leaflet-routing-geocoder-no-results", tr);
  ...
  keyPressed(e) {
    ...
    if (this.isOpen && keyCode === 13 && this.selection) {
      ...
      import_leaflet.default.DomEvent.preventDefault(e);
      return;
    }
    if (keyCode === 13) {
      import_leaflet.default.DomEvent.preventDefault(e);
  ...
  select(dir) {
    ...
    if (selection) {
      import_leaflet.default.DomUtil.removeClass(selection.firstElementChild, "leaflet-routing-geocoder-selected");
    ...
    if (selection) {
      import_leaflet.default.DomUtil.addClass(selection.firstElementChild, "leaflet-routing-geocoder-selected");
      this.selection = selection;
    }
  }
  unselect() {
    if (this.selection) {
      import_leaflet.default.DomUtil.removeClass(this.selection.firstElementChild, "leaflet-routing-geocoder-selected");
    }
    delete this.selection;
  }
  keyDown(e) {
